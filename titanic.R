...
## Exclude from model: PassengerId (arbitrary id assignment), Name, Ticket, Cabin, Embarked

## total survival rate
sum(trainData$Survived) / dim(trainData)[1] 


## Checking collinearity between numeric variables
head(trainData)
numeric_vars <- trainData %>% select(
  Age,
  SibSp,
  Parch,
  Fare
...
pairs(numeric_vars)
# We can see a negative linear association between Age and SibSp and a positive linear 
# association between Parch and SibSp.
numeric_vars %>% drop_na() %>% 
  cor()
# Both of these correlation value have a magnitude lower than 0.40 thus we would 
# classify them as weakly correlated.
# Therefore, the collinearity between them will be weak and we can still 
